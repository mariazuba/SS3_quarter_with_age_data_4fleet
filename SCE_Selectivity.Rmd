...
In this analysis, the sensitivity of the selectivity in the ECOCADIZ survey was evaluated. The baseline scenario (*S1.0_4FLEETS*) assumes a the fishery and the surveys selectivity were defined as logistic functions fixed over time. The results of this scenario show that  (Table `r #run_reference("tb_params_est")`).


```{r}
include_graphics(file.path("report/run/S1.0_4FLEETS","tb_params_est.png"))
```

Figure `r #run_bookmark("tb_params_est", figruns)`: ane.27.9a stock. Parameters estimated by the initial base model (*S1.0_4FLEETS*).
...
```{r}
include_graphics(file.path(path_mod,"tb_scenarios_Sel.png"))
```
...
```{r}
include_graphics(file.path("output/run/S1.0_4FLEETS_SelECO/plots/"  ,"sel11_timevary_surf_flt6sex1.png"))
```
...
```{r}
include_graphics(file.path("output/run/S1.0_4FLEETS_SelvarECO/plots/"  ,"sel11_timevary_surf_flt6sex1.png"))
```


```{r fig.width=5, fig.height=6, dpi=300, fig.cap= "Mean age for surveys S1.0_4FLEETS_SelECO"}
path_esc_s1<-"report/run/S1.0_4FLEETS/" 
path_esc_s2<-"report/run/S1.0_4FLEETS_SelECO/" 
path_esc_s3<-"report/run/S1.0_4FLEETS_SelvarECO/" 
# Cargar las imágenes
...
img7<- png::readPNG(file.path(path_esc_s3,"fig_meanage_fit_Pelago.png"))
img8 <- png::readPNG(file.path(path_esc_s3,"fig_meanage_fit_Ecocadiz.png"))
img9 <- png::readPNG(file.path(path_esc_s3,"fig_meanage_fit_EcocadizRecl.png"))
