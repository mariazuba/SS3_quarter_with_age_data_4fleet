...
dir.create("report/run/comparison/Wtfix", recursive = TRUE)
path_mod<-"report/run/comparison/Wtfix"

# Create a dataframe with the scenarios
list.files("boot/initial/data/run/" )
ESCs<-c("S1.0_4FLEETS", "S1.0_4FLEETS_WatageFix")

scenarios <- data.frame(
  Scenario = ESCs,
  Description =  c(
    "Watage variable by years ",
    "Watage fixed years"
  )
)
...
invisible(save_as_image(ft0, path = paste0(path_mod, "/tb_scenarios_Wtfix.png")))
...
#'*--------------------------------------------------------------------------*
esc<-c("S1.0_4FLEETS", "S1.0_4FLEETS_WatageFix")
replist<-list()
diag<-list()
params_est<-list()
Calc_Q<-list()
