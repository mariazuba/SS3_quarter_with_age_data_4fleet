...
dir.create("report/run/comparison/Sel", recursive = TRUE)
path_mod<-"report/run/comparison/Sel"

# Create a dataframe with the scenarios
list.files("boot/initial/data/run/" )
ESCs<-c("S1.0_4FLEETS", "S1.0_4FLEETS_SelECO","S1.0_4FLEETS_SelvarECO")

scenarios <- data.frame(
  Scenario = ESCs,
  Description =  c(
    "logistic fixed  for commercial fleet and all surveys",
...
invisible(save_as_image(ft0, path = paste0(path_mod, "/tb_scenarios_Sel.png")))
...
#'*--------------------------------------------------------------------------*
esc<-ESCs<-c("S1.0_4FLEETS", "S1.0_4FLEETS_SelECO","S1.0_4FLEETS_SelvarECO")
replist<-list()
diag<-list()
params_est<-list()
Calc_Q<-list()
...
Qdata$.id <- Qdata$.id <- factor(Qdata$.id,
                                 levels = ESCs<-c("S1.0_4FLEETS", "S1.0_4FLEETS_SelECO","S1.0_4FLEETS_SelvarECO"))


# Crear el gráfico
fig_q <- Qdata %>%
  ggplot(aes(x=Fleet_name, y=Calc_Q, colour=.id,group=.id)) +
  geom_point(aes(size = ifelse(.id == "S1.0", 4, 2)), shape = 21, stroke = 1.5,
             fill = ifelse(Qdata$.id == "S1.0", "black", NA)) + geom_line()+
...
ggsave(file.path(paste0("report/run/comparison/Sel","/fig_catchability2.png")), fig_q,  width=8, height=5)
...
SSplotComparisons(mod.sum, subplots=c(13,2,8,10),indexPlotEach = T,
                  ...
                  pheight = 3,png=TRUE,plotdir="report/run/comparison/Sel",legendloc='topleft')
