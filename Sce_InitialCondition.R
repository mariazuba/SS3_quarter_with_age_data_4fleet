...
dat$ctl$natM
#'*===========================================================================*
#'*Selfleet; parm=nages+1; age empiricals*
#'
old.esc<-"S1.0_4FLEETS_SelECO"
new.esc<-"S4FLEETS_SelECO_Selfleet"
dirnew<-paste0(boot,new.esc)
dat <- r4ss::SS_read(dir = paste0(boot ,new.esc))
dat$ctl$age_selex_parms #OK
#'*===========================================================================*
#'*Estima Mage breakpoint *
#'
old.esc<-"S1.0_4FLEETS_SelECO"
new.esc<-"S4FLEETS_SelECO_Mage"
dirnew<-paste0(boot,new.esc)
dat <- r4ss::SS_read(dir = paste0(boot ,new.esc))
dat$ctl$MG_parms
#'*===========================================================================*
#'*Estima M Selfleet; parm=nages+1; age empiricals (ambos se estiman)*
#'
old.esc<-"S4FLEETS_SelECO_Mage"
new.esc<-"S4FLEETS_SelECO_MageSel"
dirnew<-paste0(boot,new.esc)
dat <- r4ss::SS_read(dir = paste0(boot ,new.esc))
...
#'*===========================================================================*
#'*Estima Mfijo and  Selfleet; parm=nages+1; age empiricals (M se fija)*
#
old.esc<-"S4FLEETS_SelECO_MageSel"
new.esc<-"S4FLEETS_SelECO_MfixSel"
dirnew<-paste0(boot,new.esc)
dat <- r4ss::SS_read(dir = paste0(boot ,new.esc))
dat$ctl$age_selex_parms
dat$ctl$MG_parms
...
#'*============================================================================*
dir.create("report/run/comparison/Sel14", recursive = TRUE)
path_mod<-"report/run/comparison/Sel14"
...
invisible(save_as_image(ft0, path = paste0(path_mod, "/tb_scenarios_Sel14.png")))
...
#'*--------------------------------------------------------------------------*
esc<-c("S1.0_4FLEETS_SelECO","S4FLEETS_SelECO_Selfleet",
       "S4FLEETS_SelECO_Mage","S4FLEETS_SelECO_MageSel",
       "S4FLEETS_SelECO_MfixSel")
replist<-list()
diag<-list()
...
Qdata$.id <- Qdata$.id <- factor(Qdata$.id,
                                 levels = c("S1.0_4FLEETS_SelECO","S4FLEETS_SelECO_Selfleet",
                                            "S4FLEETS_SelECO_Mage","S4FLEETS_SelECO_MageSel",
                                            "S4FLEETS_SelECO_MfixSel"))
...
ggsave(file.path(paste0("report/run/comparison/Sel14","/fig_catchability2.png")), fig_q,  width=8, height=5)
...
SSplotComparisons(mod.sum, subplots=c(13,2,8,10),indexPlotEach = T,
                  ...
                  pheight = 3,png=TRUE,plotdir="report/run/comparison/Sel14",legendloc='topleft')
