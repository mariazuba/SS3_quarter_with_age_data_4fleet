...
dir.create("report/run/comparison/hnull", recursive = TRUE)
path_mod<-"report/run/comparison/hnull"

# Create a dataframe with the scenarios
list.files("boot/initial/data/run/" )
ESCs<-c("S1.0_4FLEETS", "S1.0_4FLEETS_hnull")
...
invisible(save_as_image(ft0, path = paste0(path_mod, "/tb_scenarios_hnull.png")))
...
#'*--------------------------------------------------------------------------*
esc<-c("S1.0_4FLEETS", "S1.0_4FLEETS_hnull")
replist<-list()
diag<-list()
params_est<-list()
Calc_Q<-list()
...
Qdata$.id <- Qdata$.id <- factor(Qdata$.id,
                                 levels = c("S1.0_4FLEETS", "S1.0_4FLEETS_WatageFix"))
...
ggsave(file.path(paste0("report/run/comparison/Wtfix","/fig_catchability2.png")), fig_q,  width=8, height=5)
...
SSplotComparisons(mod.sum, subplots=c(13,2,8,10),indexPlotEach = T,
                  ...
                  pheight = 3,png=TRUE,plotdir="report/run/comparison/Wtfix",legendloc='topleft')
