...
dir.create("report/run/comparison/Qs", recursive = TRUE)
...
scenarios <- data.frame(
  Scenario = ESCs,
  Description =  c(
    "simple $q$ linear model for all surveys",
...
#'*--------------------------------------------------------------------------*
esc<-c("S1.0_4FLEETS", "S1.0_4FLEETS_q1PEL","S1.0_4FLEETS_q1ECO", "S1.0_4FLEETS_q1BOCA", "S1.0_4FLEETS_q1ECOREC")
replist<-list()
diag<-list()
params_est<-list()
Calc_Q<-list()
...
Qdata$.id <- Qdata$.id <- factor(Qdata$.id,
                                 levels = c("S1.0_4FLEETS", "S1.0_4FLEETS_q1PEL","S1.0_4FLEETS_q1ECO", "S1.0_4FLEETS_q1BOCA", "S1.0_4FLEETS_q1ECOREC"))
...
ggsave(file.path(paste0("report/run/comparison/Qs","/fig_catchability2.png")), fig_q,  width=8, height=5)
